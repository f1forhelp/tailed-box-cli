...
func (s Server) Listen(bind string) (*Listener, error) {
	...
	listener, err := quic.ListenAddr(bind, tlsConfig, newQUICConfig())
	if err != nil {
		return nil, err
	}
...
func (l *Listener) handle(ctx context.Context, conn *quic.Conn) error {
	closeOnReturn := true
	defer func() {
		if closeOnReturn {
			_ = conn.CloseWithError(1, "handler error")
		}
	}()
	state := conn.ConnectionState().TLS
	if state.NegotiatedProtocol != tlsidentity.ALPN {
		return fmt.Errorf("%w: negotiated ALPN %q", ErrInvalidMessage, state.NegotiatedProtocol)
	}
	remote, err := metadataFromState(state)
	if err != nil {
		return err
	}
	...
	request, err := readMessage(stream)
	if err != nil {
		return err
	}
	...
	if request.NodeID != remote.NodeID || request.NetworkID != remote.NetworkID || request.Role != remote.Role {
		return fmt.Errorf("%w: ping identity does not match authenticated peer", ErrInvalidMessage)
	}
	if err := writeMessage(stream, wireMessage{
...
func Ping(ctx context.Context, paths config.Paths, endpoint string) (PingResult, error) {
	...
	start := time.Now()
	conn, err := quic.DialAddr(ctx, endpoint, tlsConfig, newQUICConfig())
	if err != nil {
		return PingResult{}, err
	}
	...
	if err := writeMessage(stream, wireMessage{Type: messageTypePing, NodeID: local.NodeID, NetworkID: local.NetworkID, Role: local.Role, Time: time.Now().UTC()}); err != nil {
		return PingResult{}, err
	}
	...
	if response.NodeID != remote.NodeID || response.NetworkID != remote.NetworkID || response.Role != remote.Role {
		return PingResult{}, fmt.Errorf("%w: pong identity does not match authenticated peer", ErrInvalidMessage)
	}
	return PingResult{
...
func metadataFromState(state tls.ConnectionState) (tlsidentity.CertificateMetadata, error) {
	if len(state.PeerCertificates) == 0 {
		return tlsidentity.CertificateMetadata{}, tlsidentity.ErrMissingCertificate
	}
	return tlsidentity.MetadataFromCertificate(state.PeerCertificates[0])
}
