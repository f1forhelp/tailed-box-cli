...
func WriteFileNew(path string, data []byte) (bool, error) {
	if path == "" {
	...
	if errors.Is(err, os.ErrExist) {
		if chmodErr := os.Chmod(path, PrivateFileMode); chmodErr != nil {
			return false, fmt.Errorf("secure existing private file %q: %w", path, chmodErr)
		}
		return false, nil
	}
	if err != nil {
		return false, fmt.Errorf("create private file %q: %w", path, err)
	}
	if _, err := file.Write(data); err != nil {
		_ = file.Close()
		return false, fmt.Errorf("write private file %q: %w", path, err)
	}
	if err := file.Close(); err != nil {
		return false, fmt.Errorf("close private file %q: %w", path, err)
	}
...
func WriteFileAtomic(path string, data []byte) (bool, error) {
	...
	tmp, err := os.CreateTemp(filepath.Dir(path), ".tailedbox-*.tmp")
	if err != nil {
